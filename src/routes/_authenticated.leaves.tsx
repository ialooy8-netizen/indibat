...
function LeavesPage() {
  ...
  const list = useQuery({
    queryKey: ["leaves"],
    queryFn: async () => {
      const { data, error } = await supabase.from("leave_requests").select("*, profiles!leave_requests_employee_id_fkey(full_name, email)").order("created_at", { ascending: false });
  ...
  // mark as seen
  if (isAdmin && list.data?.some((r) => r.unseen_admin)) {
    supabase.from("leave_requests").update({ unseen_admin: false }).eq("unseen_admin", true).then(() => qc.invalidateQueries({ queryKey: ["badge-counts"] }));
  }
