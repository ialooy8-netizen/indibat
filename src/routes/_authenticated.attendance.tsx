...
import { X, Star, AlertTriangle, ClipboardCheck, CheckCheck, MessageSquare, UserMinus, UserCheck, Clock } from "lucide-react";
...
function AttendancePage() {
    ...
    queryFn: async () => {
      ...
      if (isTeacher) {
        const { data, error } = await supabase
          .from("class_teachers").select("classes(*)").eq("teacher_id", user!.id);
        if (error) throw error;
        const rows = (data ?? [])
  ...
  /** Mark every unmarked student as present in one upsert. Smart mode default. */
  const startSession = useMutation({
    mutationFn: async () => {
      if (!students.data) return;
      const existing = att.data ?? new Map<string, string>();
      const rows = students.data
        .filter((s) => !existing.has(s.id))
        .map((s) => ({ student_id: s.id, date: today, period, status: "present", recorded_by: user?.id }));
      if (rows.length === 0) return;
      const { error } = await supabase.from("attendance").upsert(rows, { onConflict: "student_id,date,period" });
      if (error) throw error;
    },
    onSuccess: () => { toast.success("تم تسجيل الجميع حاضرين — اضغط على الغائبين فقط"); qc.invalidateQueries({ queryKey: ["attendance-day"] }); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  function notifyAllAbsent() {
    ...
    absentList.forEach((s, idx) => {
      const msg = `السلام عليكم${s.parent_name ? " " + s.parent_name : ""}، نود إعلامكم بغياب الطالب ${s.name} من صف ${className} اليوم.`;
  ...
  const stats = useMemo(() => {
    if (!students.data || !att.data) return { present: 0, absent: 0, late: 0, unset: students.data?.length ?? 0 };
    ...
    return { present, absent, late, unset: (students.data.length ?? 0) - present - absent - late };
  }, [students.data, att.data]);

  const sessionStarted = stats.unset === 0 && (students.data?.length ?? 0) > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {classId && students.data && students.data.length > 0 && (
        <>
          {!sessionStarted ? (
            <div className="glass-strong rounded-2xl p-6 text-center space-y-3 border border-primary/30">
              <p className="text-sm text-muted-foreground">الوضع الذكي — ابدأ بضغطة واحدة، ثم اضغط على الغائبين فقط.</p>
              <Button size="lg" onClick={() => startSession.mutate()} disabled={startSession.isPending} className="gap-2 gradient-primary text-primary-foreground">
                <CheckCheck className="h-5 w-5" /> {startSession.isPending ? "جاري التهيئة..." : "بدء جلسة الحضور — الكل حاضر"}
              </Button>
            </div>
          ) : (
            <div className="glass rounded-2xl p-4 flex flex-wrap items-center gap-3">
              <div className="flex gap-3 text-sm flex-1 min-w-[200px]">
                <span className="text-success">✓ {stats.present} حاضر</span>
                <span className="text-destructive">✗ {stats.absent} غائب</span>
                <span className="text-warning">⏰ {stats.late} متأخر</span>
              </div>
              {stats.absent > 0 && (
          ...
          <div className="space-y-2">
            {students.data.map((s) => {
              ...
              return (
                <div key={s.id} className={`glass rounded-xl p-3 flex items-center gap-3 ${isAbsent ? "border-r-4 border-destructive" : isLate ? "border-r-4 border-warning" : ""}`}>
                  ...
                  <Button size="sm" variant="ghost" onClick={() => setModal({ studentId: s.id, name: s.name })} title="تقييم"><Star className="h-4 w-4 text-warning" /></Button>
                  {sessionStarted ? (
                    <>
                      <Button size="sm" variant={isAbsent ? "default" : "outline"}
                        onClick={() => setStatus.mutate({ studentId: s.id, status: isAbsent ? "present" : "absent" })}
                        className={isAbsent ? "bg-destructive text-destructive-foreground" : "gap-1"}>
                        {isAbsent ? <><UserCheck className="h-4 w-4" /> رجوع</> : <><UserMinus className="h-4 w-4" /> غائب</>}
                      </Button>
                      <Button size="sm" variant={isLate ? "default" : "ghost"}
                        onClick={() => setStatus.mutate({ studentId: s.id, status: isLate ? "present" : "late" })}
                        className={isLate ? "bg-warning text-warning-foreground" : ""} title="متأخر">
