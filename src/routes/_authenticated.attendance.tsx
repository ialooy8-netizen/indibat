...
import { Check, X, Star, AlertTriangle, ClipboardCheck } from "lucide-react";
...
function AttendancePage() {
  ...
  const students = useQuery({
    ...
    enabled: !!classId,
    queryFn: async () => {
      const { data, error } = await supabase.from("students").select("id, name, behavior_points").eq("class_id", classId).order("name");
      if (error) throw error;
      return data;
    },
  });
  ...
  const att = useQuery({
    ...
    enabled: !!classId,
    queryFn: async () => {
      const { data, error } = await supabase.from("attendance").select("student_id, status").eq("date", today).eq("period", period);
      if (error) throw error;
    ...
    <div className="space-y-6">
      ...
      {classId && (
        <div className="space-y-2">
          {students.data?.map((s) => {
            ...
            return (
              <div key={s.id} className="glass rounded-xl p-3 flex items-center gap-3">
                ...
                <Button size="sm" variant="outline" onClick={() => setModal({ studentId: s.id, name: s.name })} className="gap-1"><Star className="h-3.5 w-3.5" /> تقييم</Button>
          ...
          {students.data?.length === 0 && <div className="glass rounded-2xl p-10 text-center text-muted-foreground">لا يوجد طلاب في هذا الصف</div>}
        </div>
      )}
